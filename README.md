...
the edges of the pages). It can also rasterize PDF documents that were generated from
EPUBs with Calibre (https://calibre-ebook.com/download). It is most useful when 
generating simple grayscale PDF documents that can readily be read on e-readers.
...
To convert public domain EPUBs (which you can find on https://standardebooks.org or https://gutenberg.org) 
to PDF documents using Calibre, follow these steps (Calibre will remember your settings for the next EPUB conversions):

  1- Right-click on your EPUB file, select "Open with...", and then "the main Calibre program"
  
  2- Ensure the EPUB is selected in the main window of Calibre, and then click on the "Convert books" 
     brown button at the top-left corner of the top menu.
     
  3- Select "PDF" from the "Output format:" dropdown menu in the top-right corner of the "Convert" menu.
  
  4- Click on the "Look & feel" option at the left of the "Convert" menu.
  
  5- In the "Fonts" tab of the "Look & feel" option, click on the "Choose font family" button of the "Embed font family:" 
     setting and select your font family (e.g., "Liberation Serif").
     
  6- Still in the "Fonts" tab of the "Look & feel" option, make sure the "Embed all fonts in document" and "Subset all embedded fonts" boxes are ticked.
  
  7- In the "Text" tab of the "Look & feel" option, select the "Justify text" option from the "Text justification:" drop-down menu
     should you would wish the text to be justified.
     
  8- If your EPUB only had symmetrical quotes (as is sometimes the case with some older Project Gutenberg EPUBs), you could also tick
     the "Smarten punctuation" box in the "Text" tab of the "Look & feel" option in order to get directional quotes instead.
     
  9- In the "Styling" tab of the "Look & feel" option, tick the "Fonts" box under the "Select what style information you want completely removed:" setting to
     remove any embedded fonts already present in the EPUB, so that the font you have selected to embed in the PDF in step 5 will be used to generate the PDF instead.
     
  10- Click on the "PDF output" option at the left of the "Convert" menu.
  
  11- Select the "A6" option from the "Paper size:" dropdown menu. This smaller page size (which is sized similarly to the screen of 6 to 8-inch e-readers) 
      will minimize the file size in megabytes (MB) of the rasterized PDF generated by Analog eBooks while still giving you crisp text.
      
  12- If you keep all of the pages from the original PDF document when rasterizing the PDF with Analog eBooks, or if you only remove the cover page 
      from the original PDF with Analog eBooks, you could optionally tick the "Add page numbers to the bottom of every page" box in the "PDF output" 
      option of Calibre. Any numbered pages that are removed by Analog eBooks will introduce gaps in the page numbering at the bottom of the pages, 
      hence it being important that only the cover page (which doesn't have a page number at the bottom of the page) should be removed from the original 
      PDF document using Analog eBooks, when ticking Calibre's "Add page numbers to the bottom of every page" box.
      
  13- If you have selected the "Justify text" option in step 7, tick the "Break long words at the end of lines" box to add hyphenation.
  
  14- Select the serif, sans-serif or monospace font that you have embedded in the PDF file (e.g., "Liberation Serif" under the "Serif family" 
      dropdown menu, "Liberation Sans" under the "Sans family" dropdown menu or "Liberation Mono" under the "Monospace family" dropdown menu).
      
  15- Select your favorite font type (serif, sans or mono) from the "Standard font:" dropdown menu.
  
  16- Set both the "Default font size" and "Monospace font size" to 16-20 px, with 16 px giving average paperback font sizes on 6-inch e-readers.
  
  17- Set all "Page margins" to 18 pt, which is a quarter inch, as 72 pt equals an inch.
  
  18- Click on the "OK" button at the bottom-right of the "Convert" menu.
  
  19- Once your PDF has been created ("Jobs: 0" will be written at the bottom-right of Calibre's main window), right-click
      on your title and select the "Save to disk" option. Select a path, save the file, locate and then copy and paste the
      PDF document into the "Original Book PDF File" folder in the Analog eBooks working folder.
      
  20- Run Analog eBooks and select the "[a] Generate rasterized PDF (No Crop nor Filter Modifications)" option
      from the main menu. Please read the "For EPUBs converted to PDFs:" paragraph below for more details on how 
      to use Analog eBooks from a PDF document generated from an EPUB.
...
and last pages in the 'Page Management' menu (main menu item 2). 

<b>For Scanned Book PDFs:</b>

If your starting PDF document is a scanned book PDF, I would then recommend skipping over all 
...
The Table of Contents (TOC) metadata from the original PDF file (if present) is carried over to the generated PDF file(s), 
and the page numbers adjust themselves automatically should you remove some pages or should several PDF files be generated 
from the same original PDF document.


<b>For EPUBs Converted to PDFs:</b>

If your starting PDF document was generated by converting an EPUB to a PDF document with Calibre
simply select the option "[a] Generate rasterized PDF (No Crop nor Filter Modifications)" from the 
main menu. You may wish to remove the existing cover page if the image is very dark, as it will be 
rendered in grayscale format in the final PDF document. The code automatically generates a cover page 
by default (see the "Cover Page:" section below), so you would only need to add the page number of the 
existing cover page in the original PDF document (typically 1) to the list of removed pages (option 3 of 
the "Page Management Menu", which is itself option 2 of the main menu). The default resolution is set to 
300 dots per inch (dpi), and should you wish to generate a smaller file, you could bring down the dpi 
setting to 200 dpi ("[5] Set DPI" option of the main menu). However, I had no issues reading a 300 dpi, 
200 MB, 1500-page long book on my Kobo Forma, with page turns taking around 1.5 seconds. You may change 
the maximal file size of the generated PDF in the "[3] Set Maximum PDF File Size" option of the main menu 
(default value of 100 MB). Should you find that the page turning of the PDF document is too sluggish using 
your e-reader's built-in PDF reader software, you could install KOReader on your e-reader (see the KOReader 
section at the end of the README file) and read the PDF with KOReader instead for almost instantaneous page 
turns. The Table of Contents (TOC) metadata from the original PDF file (if present) is carried over to the 
generated PDF file(s), and the page numbers adjust themselves automatically should you remove some pages or 
should several PDF files be generated from the same original PDF document. 

<b>Description of the Settings:</b>
...
- Filters: There are three stages where pixels are filtered out in order to remove the background color or
  ...
  3- Full-Page Filter: This filter applies to the full page, but unlike the Page Color filter, the distribution
   ...
   - Image Mode: The 'Image Mode' setting will set a cap on the pixels that will be set to white when 
     filtering out the page color and applying the full-page filter step afterwards. This mode will 
     prevent pages with images, which have an average grayscale pixel value that is significantly 
     darker than pages containing only text, from becoming faded during these filter steps 
     (default setting: False).
     
     The 'Image Mode Multiplier' will determine the number of standard deviations (the number may 
     be positive or negative, and may contain decimals) that will be added to the mean threshold 
     above which the pixels will be set to white when filtering out the page color and applying the 
     full-page filter afterwards. The threshold values are distributed within 3 standard deviations 
     on either side of the mean. A value of 'Image Mode Multiplier' of zero will give the mean 
     threshold as a threshold, while positive values up to +3.0 will keep more and more original 
     pixels, and negative values -3.0 and over will filter out pixels more aggressively 
     (default setting: -0.85).
